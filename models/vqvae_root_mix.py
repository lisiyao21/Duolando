...
class VQVAERmix(nn.Module):
    ...
    def _decode(self, zs, start_level=0, end_level=None):
        ...
        decoder_pos, decoder, decoder_root, x_quantised = self.decoders[start_level], self.decoders_rot[start_level], self.decoders_root[start_level], xs_quantised[0:1]
        ...
        return x_out_pos, x_out, x_vel_out
    ...
    def decode(self, zs, start_level=0, end_level=None, bs_chunks=1):
        ...
        for i in range(bs_chunks):
            zs_i = [z_chunk[i] for z_chunk in z_chunks]
            x_pos, x_out, x_vel = self._decode(zs_i, start_level=start_level, end_level=end_level)
            x_outs.append(x_out)
            x_pos_outs.append(x_pos)
            x_vel_outs.append(x_vel)
        return t.cat(x_pos_outs, dim=0), t.cat(x_outs, dim=0), t.cat(x_vel_outs, dim=0)
    ...
    def forward(self, x, x_rot, x_shift):
        ...
        for level in reversed(range(self.levels)):
            ...
            this_recons_loss = _loss_fn(x_target_pos, x_out_pos) + _loss_fn(x_target, x_out) +  _loss_fn(x_target_shift, x_out_vel)
            ...
            acceleration_loss += _loss_fn(x_out_pos[:, 2:] + x_out_pos[:, :-2] - 2 * x_out_pos[:, 1:-1], x_target_pos[:, 2:] + x_target_pos[:, :-2] - 2 * x_target_pos[:, 1:-1]) \
                ...
                _loss_fn(x_target_shift[:, 1:] - x_target_shift[:, :-1], x_out_vel[:, 1:] - x_out_vel[:, :-1]) 
        ...
        return x_out_pos, x_out, x_out_vel, loss, metrics
